...
class BirdCNN(nn.Module):
    ...
    def __init__(self, emb_dim=EMBEDDING_DIM, dropout: float = 0.3):
        super().__init__()
...
# Training
def train_one_epoch(model, supcon_loss, loader, optimizer, device, noise_std=0.02):
...
def run_train(args):
    ...
    try:
        sampler = BalancedBatchSampler(train_ds.labels, n_classes=min(num_classes, args.supcon_n_classes), n_samples=args.supcon_n_samples)
        train_dl = DataLoader(train_ds, batch_sampler=sampler, num_workers=NUM_WORKERS)
        ...
        train_dl = DataLoader(train_ds, batch_size=args.batch_size, shuffle=True, num_workers=NUM_WORKERS, drop_last=True)
