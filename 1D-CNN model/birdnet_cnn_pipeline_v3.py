...
INPUT_DIM = 1024  # BirdNet embedding dim
EMBEDDING_DIM = 256  # Output embedding dim
...
SUPCON_TEMP = 0.07  # temperature τ
SUPCON_N_CLASSES = 8  # individuals sampled per balanced batch
SUPCON_N_SAMPLES = 4  # segments sampled per individual per batch


def _robust_basename(p):  # Helper function to return the correct path
    p = str(p)
    return ntpath.basename(p) if '\\' in p else posixpath.basename(p)


def load_embeddings_csv(csv_path, labels_csv=None):
    # Load a BirdNet feature-embedding CSV, with col "embedding" a comma-separated string of 1024 floats
    ...
    if labels_csv is not None:
        ...
        if ldf_path_col is None:
            ...
            if not other_cols:
                raise ValueError("labels_csv must have a file path column and an 'individual_id' column.")
        ...
        if matched < len(df):
            ...
            if ldf['_basename'].duplicated().any():
                ...
                print(f"[WARNING] Duplicate basenames in labels_csv: {dups}. " "Using first match.")
        ...
        if matched == 0:
            ...
            raise ValueError("Label merge produced 0 matches. " "The 'file_path' values in your embeddings CSV and labels CSV do not overlap. " "Check that both CSVs refer to the same audio files.")

        if matched < len(df):
            unmatched = merged[merged['individual_id'].isna()]['file_path'].tolist()
            print(f"[WARNING] {len(unmatched)} rows could not be matched to a label " f"and will be dropped:\n  {unmatched[:5]}" + (" …" if len(unmatched) > 5 else ""))
...
class EmbeddingDataset(Dataset):
    # One sample per row (segment).
    def __init__(self, df, label_encoder=None):
        if df['individual_id'].eq('').any():
            raise ValueError("Some rows have empty individual_id. " "Provide labels via --labels_csv or add an individual_id column.")
...
class BalancedBatchSampler(Sampler):  # Yields batches of n_classes × n_samples indices so that every class in the batch is represented at least n_samples times.

    def __init__(self, labels, n_classes=SUPCON_N_CLASSES, n_samples=SUPCON_N_SAMPLES):
        ...
        self.valid_classes = [lbl for lbl, idxs in self.label_to_idxs.items() if len(idxs) >= n_samples]
        if len(self.valid_classes) < 2:
            raise ValueError(f"BalancedBatchSampler needs ≥2 classes with ≥{n_samples} samples each. " f"Found {len(self.valid_classes)} qualifying class(es). " f"Reduce --supcon_n_samples or provide more training data.")
        self.n_batches = max(1, len(self.labels) // self.batch_size)

    def __iter__(self):
        for _ in range(self.n_batches):
            chosen = random.sample(self.valid_classes, min(self.n_classes, len(self.valid_classes)))
            batch = []
            for cls in chosen:
...
# Feature augmentation of adding noise/ dropout
def augment_vec(x, noise_std=0.02):
...
class ConvBlock1d(nn.Module):
    def __init__(self, in_ch, out_ch, kernel, stride, padding):
        super().__init__()
        self.net = nn.Sequential(nn.Conv1d(in_ch, out_ch, kernel_size=kernel, stride=stride, padding=padding, bias=False), nn.BatchNorm1d(out_ch), nn.GELU())
...
class BirdCNN(nn.Module):
    ...
    def __init__(self, emb_dim=EMBEDDING_DIM, dropout=0.3):
        super().__init__()
        self.encoder = nn.Sequential(ConvBlock1d(1, 64, kernel=8, stride=4, padding=2), ConvBlock1d(64, 128, kernel=4, stride=2, padding=1), ConvBlock1d(128, 256, kernel=4, stride=2, padding=1), ConvBlock1d(256, 256, kernel=4, stride=2, padding=1), nn.AdaptiveAvgPool1d(1))  # [B, 256, 1]
        self.head = nn.Sequential(nn.Dropout(p=dropout), nn.Linear(256, emb_dim, bias=False), nn.BatchNorm1d(emb_dim))
        self.emb_dim = emb_dim

    def forward(self, x):
        # x : [Batch, 1024] to [Batch, 256]
...
# Training
def train_one_epoch(model, supcon_loss, loader, optimizer, device, noise_std: float = 0.02):
...
# Checkpoint helpers
def save_checkpoint(path, model, le, epoch, prototypes):
    torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'le_classes': le.classes_, 'emb_dim': model.emb_dim, 'prototypes': prototypes}, path)
...
def load_checkpoint(path, device=DEVICE):
    ...
    print(f"Loaded checkpoint: epoch={ck['epoch']}, " f"emb_dim={emb_dim}, num_classes={len(le.classes_)}, " f"classes={list(le.classes_)}")
    return model, le, prototypes


# Batched inference
@torch.no_grad()
def embed_vecs(model, vecs_np: np.ndarray, device, batch_size=256):
    # Push raw 1024-d feature vectors through the CNN. Returns 256-d normalised embeddings [N, 256].
    model.eval()
    out = []
    for start in range(0, len(vecs_np), batch_size):
        chunk = torch.from_numpy(vecs_np[start : start + batch_size]).float().to(device)
        out.append(model(chunk).cpu().numpy())
    return np.concatenate(out, axis=0)


@torch.no_grad()
def compute_prototypes(model, dataset: EmbeddingDataset, device, batch_size=256):
...
# Validation accuracy
@torch.no_grad()
def evaluate_accuracy(model, train_ds, val_ds, device):

    # Compute prototype nearest-neighbour accuracy on val_ds.
    # Prototypes are built from train_ds

    prototypes = compute_prototypes(model, train_ds, device)  # [C, D]
    vecs = torch.from_numpy(val_ds.vecs).float().to(device)
    embs = model(vecs).cpu().numpy()  # [N, D]
    preds = (embs @ prototypes.T).argmax(axis=1)  # [N]
    return float((preds == val_ds.labels).mean())
...
def run_train(args):
    ...
    if df['individual_id'].eq('').any():
        raise ValueError("Training requires individual_id for every row. " "Add an individual_id column to your embeddings CSV " "or provide --labels_csv.")
    ...
    # Phase 1: stratified 85/15 split to find best epoch
    # Split per individual so every individual stays in training
    VAL_FRAC = 0.15
    train_idx, val_idx = [], []
    ...
    df_tr = df.loc[train_idx].reset_index(drop=True)
    df_val = df.loc[val_idx].reset_index(drop=True)
    print(f"Split → train: {len(df_tr)} segments  |  val: {len(df_val)} segments (15%)\n")

    train_ds = EmbeddingDataset(df_tr, label_encoder=le)
    val_ds = EmbeddingDataset(df_val, label_encoder=le)

    def _make_loader(ds):
        try:
            sampler = BalancedBatchSampler(ds.labels, n_classes=min(num_classes, args.supcon_n_classes), n_samples=args.supcon_n_samples)
            print(f"SupCon batch: {sampler.n_classes} classes × " f"{sampler.n_samples} samples = {sampler.batch_size} per batch")
            return DataLoader(ds, batch_sampler=sampler, num_workers=NUM_WORKERS)
        except ValueError as e:
            print(f"[WARNING] BalancedBatchSampler: {e}")
            print("[WARNING] Falling back to standard DataLoader — " "some batches may lack positive pairs.")
            return DataLoader(ds, batch_size=args.batch_size, shuffle=True, num_workers=NUM_WORKERS, drop_last=True)
    ...
    # Training loop with val accuracy
    best_acc, best_epoch = 0.0, 1
    for epoch in range(1, args.epochs + 1):
        tr_loss = train_one_epoch(model, supcon, train_dl, optimizer, DEVICE)
        scheduler.step()
        val_acc = evaluate_accuracy(model, train_ds, val_ds, DEVICE)
        marker = " ◄ best" if val_acc > best_acc else ""
        print(f"Epoch {epoch:3d}/{args.epochs}  supcon_loss={tr_loss:.4f}" f"  val_acc={val_acc:.4f}{marker}")
        if val_acc > best_acc:
    ...
    print(f"\nRetraining on full dataset for {best_epoch} epoch(s) …")
    full_ds = EmbeddingDataset(df, label_encoder=le)
    full_dl = _make_loader(full_ds)
    model = BirdCNN(emb_dim=EMBEDDING_DIM).to(DEVICE)
    optimizer = optim.AdamW(model.parameters(), lr=args.lr, weight_decay=WEIGHT_DECAY)
    scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=best_epoch)

    for epoch in range(1, best_epoch + 1):
        tr_loss = train_one_epoch(model, supcon, full_dl, optimizer, DEVICE)
        scheduler.step()
        print(f"Epoch {epoch:3d}/{best_epoch}  supcon_loss={tr_loss:.4f}")

    # Compute prototypes over full training set
    print("\nComputing per-individual prototype embeddings …")
    prototypes = compute_prototypes(model, full_ds, DEVICE)
    ...
    save_checkpoint(final_path, model, le, best_epoch, prototypes)
...
def run_test(args):
    ...
    for fp, grp in df.groupby('file_path', sort=False):
        ...
        pred_lbl = str(le.classes_[pred_idx]) if pred_idx < len(le.classes_) else f"unknown_{pred_idx}"
        confidence = float((votes == pred_idx).mean())
        results.append({'file_path': fp, 'predicted_individual': pred_lbl, 'confidence': f"{confidence:.2f}"})
...
def run_embedding(args):
    ...
    pd.DataFrame({'file_path': file_paths, 'emb_row': range(len(file_paths))}).to_csv(meta_csv, index=False)

    print(f"\nEmbeddings saved → {args.out_emb_npy}  shape={file_embs_arr.shape}")
    print(f"Metadata  saved → {meta_csv}")
    print("Each row is a 256-d L2-normalised identity embedding for one file.")


# CLI
def main():
    parser = argparse.ArgumentParser(description="BirdNet 1024-d feature CNN pipeline for individual recognition", formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('--mode', required=True, choices=['train', 'test', 'embedding'], help=("train    : Train CNN on labelled BirdNet embeddings\n" "test     : Predict individual ID for unlabelled files\n" "embedding: Extract 256-d identity vectors"))

    # Data
    parser.add_argument('--embeddings_csv', type=str, required=True, help="Path to BirdNet feature CSV (file_path, start, end, embedding)")
    parser.add_argument('--labels_csv', type=str, default=None, help="(Optional) Separate CSV (file_path, individual_id) for training labels.\n" "Not needed if individual_id column is already in --embeddings_csv.")
    parser.add_argument('--val_embeddings_csv', type=str, default=None, help="Validation set embeddings CSV (reserved for future use)")
    parser.add_argument('--val_labels_csv', type=str, default=None, help="Validation set labels CSV (reserved for future use)")

    # Training
    parser.add_argument('--epochs', type=int, default=EPOCHS)
    parser.add_argument('--batch_size', type=int, default=64, help="Fallback batch size if BalancedBatchSampler cannot be used")
    parser.add_argument('--lr', type=float, default=LR)
    parser.add_argument('--supcon_temp', type=float, default=SUPCON_TEMP, help=f"SupCon temperature τ (default {SUPCON_TEMP}). " "Lower = harder negatives / tighter clusters.")
    parser.add_argument('--supcon_n_classes', type=int, default=SUPCON_N_CLASSES, help=f"Individuals per balanced batch (default {SUPCON_N_CLASSES})")
    parser.add_argument('--supcon_n_samples', type=int, default=SUPCON_N_SAMPLES, help=f"Segments per individual per batch (default {SUPCON_N_SAMPLES})")
    parser.add_argument('--checkpoint_dir', type=str, default=CHECKPOINT_DIR)

    # Inference
    parser.add_argument('--checkpoint', type=str, default=None, help="Path to trained .pt checkpoint (test / embedding modes)")
