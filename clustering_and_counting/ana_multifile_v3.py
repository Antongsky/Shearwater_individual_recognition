...
n_components = 2
PLOT      = True   # set False to skip plots, otherwise show diagnostic plots
...
def predict_from_scores(cluster_vec, score_vec, window_frac=0.2, polyorder=3):
    ...
    # Smooth the silhouette curve with a Savitzky-Golay filter and return point prediction and range prediction.
    '''Point prediction:
      - Find the global maximum.
      - Find any local maxima that occur before (left of) the global maximum.
      - If any such local maximum has height >= 0.90 * global_max,
        use the first one as the point prediction; otherwise use the global maximum.
    Range prediction:
      - All k values where smoothed score >= 0.90 * global_max.
    '''
    ...
    global_max = np.max(score_s)
    global_idx = int(np.argmax(score_s))
    threshold  = global_max * 0.90

    # Find local maxima in the smoothed curve
    local_peaks, _ = find_peaks(score_s)

    # Keep only local maxima before the global max
    # also meeting the 90% height threshold
    early_peaks = [p for p in local_peaks
                   if p < global_idx and score_s[p] >= threshold]

    point_idx  = early_peaks[0] if early_peaks else global_idx
    point_pred = int(cluster[point_idx])
